...
I tried to run a federated query from the LDWG endpoint to the syriaca.org endpoint, but for some reason, the syriaca.org endpoint identifies the Content-type: that it returns as text/xml rather than the expected application/sparql-results+xml .  That results in an error, making it impossible to run the query from the LDWG endpoint's side.

Running the query the other way, I ran into the error "unable to find valid certification path to requested target". (There is also an odd error that occurs at the syriaca.org endpoint when there is only one variable listed in the SELECT clause.)
